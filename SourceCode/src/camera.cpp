...
Camera::Camera(const cm::Vec2u& resolution, const CameraConfig& config)
  : origin(config.origin), backward(cm::normalize(-config.view_dir))
{
  // camera coordinate system
  right = cm::normalize(cm::cross(config.up, backward));
  up = cm::cross(backward, right);
  // match sensor aspect ratio to resolution aspect ratio
  float sensor_height = (float(resolution.y) / float(resolution.x)) * config.sensor_width;
  // pixel size is always quadratic: (sensor_height / float(resolution.y) = sensor_width / float(resolution.x))
  pixel_size = config.sensor_width * (1.0 / float(resolution.x));
  upper_left_corner = origin + (sensor_height / 2.0) * up - (config.sensor_width / 2.0) * right - config.focal_length * backward;
}
