...
SpatialConfiguration::SpatialConfiguration(const cm::Quat<float>& orientation, const cm::Vec3& position) : orientation(orientation), position(position)
{
  update_coordinate_system();
}
...
void SpatialConfiguration::rotate(float yaw, float pitch, float roll, bool use_radian)
{
  ...
  cm::Quat q_yaw = cm::angle_axis(yaw, local_y_axis);
  cm::Quat q_pitch = cm::angle_axis(pitch, local_x_axis);
  cm::Quat q_roll = cm::angle_axis(roll, local_z_axis);
...
void SpatialConfiguration::set_orientation(const cm::Quat<float>& orientation)
{
  this->orientation = orientation;
  update_coordinate_system();
}

cm::Quat<float> SpatialConfiguration::get_orientation() const
{
  return orientation;
}
...
{
  const cm::Quat orientation = cm::slerp(a.get_orientation(), b.get_orientation(), weight);
...
{
  const cm::Quat q_x_axis = cm::normalize(orientation * cm::Quat<float>(0.0f, x_axis) * cm::conjugate(orientation));
  const cm::Quat q_y_axis = cm::normalize(orientation * cm::Quat<float>(0.0f, y_axis) * cm::conjugate(orientation));
  const cm::Quat q_z_axis = cm::normalize(orientation * cm::Quat<float>(0.0f, z_axis) * cm::conjugate(orientation));
